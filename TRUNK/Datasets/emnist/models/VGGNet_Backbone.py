...
class MNN(nn.Module):
	...
	def _make_layer(self, input_channel):
		...
		if(self.supergroup == "root"):
			...
			###
			layers.append(nn.Conv2d(in_channels=16, out_channels=16, kernel_size=3, padding=1))
			layers.append(nn.BatchNorm2d(num_features=16))
			layers.append(nn.ReLU(inplace=True))
			###
			...
			layers.append(nn.ReLU(inplace=True))
			###
			layers.append(nn.Conv2d(in_channels=32, out_channels=48, kernel_size=3, padding=1))
			layers.append(nn.BatchNorm2d(num_features=48))
			layers.append(nn.ReLU(inplace=True))
			###
			layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
